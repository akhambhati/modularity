"""
Python adaptation of the GenLouvain Algorithm.

This version is heavily based on:
...
def genlouvain(ml_mod_matr, limit, verbose=True):
    ...
    err.check_type(ml_mod_matr, np.ndarray)
    ...
    while True:
        ...
        for count in range(limit):
            ...
            vbprint('   --- Found %4d communities in %.5f sec \r' %
                    (i_phaseCount, t_phaseEnd - t_phaseStart))

        comm_id = _tidyconfig(comm_id)
        for i in range(n_comm_id):
